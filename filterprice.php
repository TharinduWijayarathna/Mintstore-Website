...
$all = Database::search("SELECT * FROM `product` WHERE `price` >= '".$from."' AND `price` <= '".$to."' AND `status_id`='1'");
$pnum = $all->num_rows;

$number_of_pages = ceil($pnum / $result_per_page);
$page_first_result = ((int)$pageno - 1) * $result_per_page;

$fprice = Database::search("SELECT * FROM `product` WHERE `price` >= '".$from."' AND `price` <= '".$to."' AND `status_id`='1' LIMIT " . $result_per_page . " OFFSET " . $page_first_result . " ");
$det = $fprice->num_rows;

if ($det >= 1) {

    while ($p = $fprice->fetch_assoc()) {
