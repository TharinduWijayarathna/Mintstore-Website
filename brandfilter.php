...
$product = Database::search("SELECT * FROM `product` WHERE `model_has_brand_id` IN (SELECT `id` FROM `model_has_brand` WHERE `brand_id`='" . $id . "')");
$productnm = $product->num_rows;

$number_of_pages = ceil($productnm / $result_per_page);
$page_first_result = ((int)$pageno - 1) * $result_per_page;

$selectedrs = Database::search("SELECT * FROM `product` WHERE `model_has_brand_id` IN (SELECT `id` FROM `model_has_brand` WHERE `brand_id`='" . $id . "') LIMIT " . $result_per_page . " OFFSET " . $page_first_result . " ");
