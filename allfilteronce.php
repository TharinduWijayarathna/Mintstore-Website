...
if ($bid == "0") {
    ...
    $product = Database::search("SELECT * FROM `product` WHERE `price` >= '" . $from . "' AND `price` <= '" . $to . "' AND `status_id`='1' AND `condition_id` = '" . $cond . "' ");
    $productnm = $product->num_rows;

    $number_of_pages = ceil($productnm / $result_per_page);
    $page_first_result = ((int)$pageno - 1) * $result_per_page;

    $selectedrs = Database::search("SELECT * FROM `product` WHERE `price` >= '" . $from . "' AND `price` <= '" . $to . "' AND `status_id`='1' AND `condition_id` = '" . $cond . "' LIMIT " . $result_per_page . " OFFSET " . $page_first_result . " ");
...
?>
    ...
    $product = Database::search("SELECT * FROM `product` WHERE `price` >= '" . $from . "' AND `price` <= '" . $to . "' AND `status_id`='1' AND `condition_id` = '" . $cond . "' AND `model_has_brand_id` IN (SELECT `id` FROM `model_has_brand` WHERE `brand_id`='" . $bid . "') ");
    $productnm = $product->num_rows;

    $number_of_pages = ceil($productnm / $result_per_page);
    $page_first_result = ((int)$pageno - 1) * $result_per_page;

    $selectedrs = Database::search("SELECT * FROM `product` WHERE `price` >= '" . $from . "' AND `price` <= '" . $to . "' AND `status_id`='1' AND `condition_id` = '" . $cond . "' AND `model_has_brand_id` IN (SELECT `id` FROM `model_has_brand` WHERE `brand_id`='" . $bid . "') LIMIT " . $result_per_page . " OFFSET " . $page_first_result . " ");
    $n =  $selectedrs->num_rows;
